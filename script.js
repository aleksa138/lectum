...
function renderFiles(files) {
    ...
    if (!files || files.length === 0) {
        list.innerHTML = '<div style="color:var(--text-muted); text-align:center; padding:20px;">Пусто</div>';
        return;
    }
    files.forEach((file) => {
        const div = document.createElement('div');
        ...
        div.innerHTML = `
            <div class="file-left" onclick="previewFile('${file.id}')">
            ...
            <div style="display:flex; gap:5px;">
                ${!file.importedFrom ? `<button class="btn btn-outline" style="padding:4px 8px; font-size:0.7rem;" onclick="shareFile(event, '${file.id}')" title="Поделиться">🔗</button>` : ''}
...
function previewFile(file) {
    ...
    if (file.type.includes('pdf')) {
        box.innerHTML = `<div style="color:white; text-align:center;"><h3>📄 ${file.name}</h3><br><a href="${file.blobUrl}" target="_blank" class="btn">Открыть PDF</a></div>`;
...
// === ШАРИНГ И ИМПОРТ (ИСПРАВЛЕНО) ===

// Улучшенная функция кодирования (использует URI Component для безопасности)
